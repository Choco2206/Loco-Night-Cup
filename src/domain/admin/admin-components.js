...
const ADMIN_CATEGORIES = {
  ...
  tests: {
    ...
    actions: [
      ...
      ['admin_fc27_ceremony_test', 'FC-27-Grafiken testen', 'Sieben Tagesbilder und die neuen Special Awards mit unterschiedlichen Teams prüfen'],
...
function buildAdminPanelPayload(selectedCategory = null) {
  ...
  if (selectedCategory === 'tests') {
    components.push(new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId('admin_fc27_ceremony_test').setLabel('FC-27-Grafiken testen').setEmoji('🏆').setStyle(ButtonStyle.Primary)
    ));
  }
