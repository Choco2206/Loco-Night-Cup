...
const ADMIN_SELECT_PREFIXES = [
  ...
  'admin_power_ranking_test_team_select:',
];
...
const POWER_RANKING_TEST_PAGE_SIZE = 25;
...
function buildPowerRankingTestTeamSelectPayload(page = 0) {
  const teams = sortedRegisteredTeams();
  if (!teams.length) throw new Error('Es gibt keine auswählbaren Teams.');

  const totalPages = Math.max(1, Math.ceil(teams.length / POWER_RANKING_TEST_PAGE_SIZE));
  const currentPage = clampPage(page, totalPages);
  const pageTeams = teams.slice(
    currentPage * POWER_RANKING_TEST_PAGE_SIZE,
    (currentPage + 1) * POWER_RANKING_TEST_PAGE_SIZE
  );
  const components = [new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId(`admin_power_ranking_test_team_select:${currentPage}`)
  ...
  if (totalPages > 1) {
    components.push(new ActionRowBuilder().addComponents(
      new ButtonBuilder()
        .setCustomId(`admin_power_ranking_test_page:${currentPage - 1}`)
        .setLabel('Zurück')
  ...
  return {
    content: totalPages === 1
      ? 'Team für den Champion-der-Woche-Test auswählen.'
...
async function handleAdminSelect(interaction, client, settings) {
  ...
  if (interaction.customId.startsWith('admin_power_ranking_test_team_select:')) {
    const teamId = interaction.values?.[0];
    await interaction.deferUpdate();
    const result = await postPowerRankingChampionTest({ guild: interaction.guild, teamId });
    await interaction.editReply({
      content: `Champion-der-Woche-Test für **${result.teamName}** wurde in <#${result.channelId}> gepostet.`,
      components: [],
    });
    return true;
  }
...
async function handleAdminInteraction(interaction, client) {
  ...
  try {
    ...
    if (actionCustomId.startsWith('admin_power_ranking_test_page:')) {
      const [, page] = actionCustomId.split(':');
      await interaction.update(buildPowerRankingTestTeamSelectPayload(page));
      return true;
    }
    ...
    if (actionCustomId === 'admin_power_ranking_champion_test') {
      await interaction.reply({
