...
test('renders a mobile-safe table with points explanation', () => { const pages = rankingPages([{ ...snapshotTeam(team()), rank: 1 }], '2026-08-06T12:00:00.000Z'); assert.match(pages[0], /LOCO LEGACY RANKING/); assert.match(pages[0], /Sieg 3 Punkte/); assert.match(pages[0], /CUPS\s+SP/); assert.ok(pages.every(page => page.length <= 2000)); });
